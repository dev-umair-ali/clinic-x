...
function PrescriptionStatCard({ title, value, change, changeType, icon }: PrescriptionStatCardProps) {
  const changeColorClass = changeType === "positive" ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"
  
  return (
    <Card className="shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="flex items-center gap-2">
          <div className="rounded-full p-2 bg-muted text-muted-foreground">{icon}</div>
  ...
  return (
    <div className="bg-card p-4 rounded border border-border">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-foreground">Prescriptions Trends</h2>
        <DropdownMenu>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <Card className="shadow-sm">
          <CardHeader>
            <CardTitle className="text-sm font-medium text-muted-foreground">Most Prescribed</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-foreground">Metformin</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="shadow-sm p-4">
        <LineChartWithArea
          ...
          totalColor="hsl(var(--custom-dashboard-green-chart1))"
          pendingColor="hsl(var(--custom-dashboard-blue-chart1))"
        />
        <div className="flex justify-center gap-6 mt-4 text-sm text-muted-foreground">
          <div className="flex items-center">
            <span className="h-3 w-3 rounded-full bg-[#1DA68F] mr-2" /> Total Prescriptions
          </div>
          <div className="flex items-center">
            <span className="h-3 w-3 rounded-full bg-[#0E84EB] mr-2" /> Pending Refills
          </div>
        </div>
      </Card>
    </div>
