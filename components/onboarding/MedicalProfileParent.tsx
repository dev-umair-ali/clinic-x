...
interface MedicalProfileParentProps {
  onNext: () => void;
}

const MedicalProfileParent = ({ onNext }: MedicalProfileParentProps) => {
  const { toast } = useToast();
  const dispatch = useDispatch<AppDispatch>();
  const { user } = useSelector((state: any) => state.auth);
  const { medicalProfile, isLoading, error } = useSelector(
    (state: any) => state.onboarding,
  );
  const [formData, setFormData] = React.useState<any>(medicalProfile || {});
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Toaster />
      <MedicalProfile formData={formData} updateFormData={handleInputChange} />
      <button
        type="submit"
        className="
    mt-6
...
export default MedicalProfileParent;
