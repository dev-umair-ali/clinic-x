...
import { fetchClinicAuditLogsForAssistant, fetchAuditLog, clearSelectedLog } from "@/lib/slices/auditLogSlice";
...
export default function LogsPage() {
  ...
  useEffect(() => {
    ...
    params.clinicRef = (user?.clinicRef as any)?._id || "";

    dispatch(fetchClinicAuditLogsForAssistant(params));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[hsl(var(--background))] to-[hsl(var(--background))] p-6 md:p-8">
      <div className="mb-8">
          ...
          <div className="px-4 sm:px-6 py-4 border-t border-[hsl(var(--border))] bg-[hsl(var(--muted)/0.1)]">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-5">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <SelectTrigger className="h-9 w-20 text-sm rounded-lg border-[hsl(var(--color-brand-teal))] focus:ring-[hsl(var(--color-brand-teal))]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="rounded-lg shadow-md bg-[hsl(var(--background))] border-[hsl(var(--border))]">
                    {[10, 20, 30, 40, 50].map((s) => (
                      <SelectItem
                        key={s}
                        value={String(s)}
                        className="text-sm rounded-md hover:bg-[hsl(var(--color-brand-teal)/0.1)] focus:bg-[hsl(var(--color-brand-teal)/0.2)]"
                      >
                        {s}
                      </SelectItem>
                    ))}
                  ...
                  <div className="space-y-2">
                    {selectedLog.changes.map((change, idx) => (
                      <div key={idx} className="text-xs">
                        <span className="font-medium text-[hsl(var(--popover-foreground))]">{change.field}:</span>
