...
export default function AddPatientStyled() {
  ...
  useEffect(() => {
    if (formData.clinicRef) {
      ...
      // Clear doctors if no clinic selected
      dispatch(clearClinicDoctors());
    }
  }, [dispatch, formData.clinicRef]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await dispatch(
        createPatient(payload as unknown as CreatePatientRequest)
      );
      if (res.meta.requestStatus === "fulfilled") {
        toast({ title: "Success", description: "Patient added!" });
        router.push("/admin/patients/view/" + (res.payload as any)._id);
      }
      else {
        toast({
          title: "Error",
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => {
                        setFormData({ ...formData, clinicRef: v, doctorRef: "" });
                      }}
                    >
                      <SelectTrigger className=" dark:text-[hsl(var(--foreground))]">
