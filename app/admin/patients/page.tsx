...
export default function AdminPatientsPage() {
  ...
  useEffect(() => {
    ...
    if (doctorsFilter && doctorsFilter !== "all") params.doctorId = doctorsFilter;
  ...
  }, [dispatch, searchQuery, statusFilter, clinicFilter, doctorsFilter, currentPage, itemsPerPage]);
  ...
  useEffect(() => {
    ...
    if (clinicFilter && clinicFilter !== "all") {
      dispatch(fetchClinicDoctors(clinicFilter));
    } else {
      dispatch(fetchClinicDoctors(""));
    }
  }, [dispatch, clinicFilter]);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-[hsl(var(--foreground))] dark:text-[hsl(var(--foreground))] mb-2">
                  Patients
                </h1>
              ...
              <button
                onClick={() => handleNavigation("/admin/patients/add")}
                className="px-4 py-2 bg-[hsl(var(--color-brand-teal))] min-w-[130px] text-white rounded hover:bg-[hsl(var(--color-brand-teal-dark))] flex justify-center items-center gap-2 font-bold text-sm transition-colors mt-4 sm:mt-0"
              >
                + Add Patient
              </button>
            </div>
            ...
            <div className="bg-[hsl(var(--card))] rounded-lg shadow-sm border border-[hsl(var(--border))] p-4 mb-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
               ...
               {/* Doctor Filter */}
                <div>
                  <Select value={doctorsFilter} onValueChange={handleDoctorChange}>
                    <SelectTrigger>
              ...
              {(searchQuery || statusFilter !== "all" || clinicFilter !== "all" || doctorsFilter !== "all") && (
                <div className="mt-4 flex items-center gap-2">
                  ...
                  {clinicFilter !== "all" && (
                   ...
                   {doctorsFilter !== "all" && (
                    <span className="inline-flex items-center px-2 py-1 rounded-md text-xs bg-[hsl(var(--color-brand-teal)/0.1)] text-[hsl(var(--color-brand-teal))]">
                      Doctor: {clinicDoctors.find(d => d._id === doctorsFilter)?.firstName} {clinicDoctors.find(d => d._id === doctorsFilter)?.lastName}
                    </span>
                  )}
                  <Button
                    variant="ghost"
          ...
          {error && patientsData?.length === 0 && (
            <div className="mb-4 bg-[hsl(var(--color-status-error-light))] border border-[hsl(var(--color-status-error))] text-[hsl(var(--color-status-error))] px-4 py-3 rounded-full text-sm">
              {error}
            </div>
          )}
          ...
          {loading ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-[hsl(var(--muted-foreground))]">Loading patients...</span>
            </div>
          ) : (
            <PatientTable
