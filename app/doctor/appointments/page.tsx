...
export default function DoctorAppointments() {
  ...
  const userId = authUser?.doctorId;
  ...
  const fetchList = async () => {
    if (authUser?.doctorId) {
      ...
      setError(null);
      try {
        const res = await appointmentService.getDoctorPersonalAppointments(authUser.doctorId);
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    if (status === "start_visit") {
      ...
      if (appointment) {
        ...
        if (patientId) {
          router.push(`/doctor/appointments/start-visit?patientId=${patientId}&appointmentId=${id}`);
        }
      }
    }
