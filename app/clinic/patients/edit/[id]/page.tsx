...
import { updateClinicPatient } from "@/lib/slices/clinicPatientSlice";
...
import type { UpdateClinicPatientRequest } from "@/lib/api/services/clinicPatientService";
...
export default function EditPatientPage() {
  ...
  const { clinicPatient } = useSelector(
  ...
  useEffect(() => {
    if (!clinicPatient) return;
    setFormData({
      firstName: clinicPatient.firstName || "",
      lastName: clinicPatient.lastName || "",
      email: clinicPatient.email || "",
      phoneNumber: clinicPatient.phoneNumber || "",
      age: clinicPatient.age?.toString() || "",
      gender: clinicPatient.gender || "",
      profilePicture:
        clinicPatient.profilePicture || "/placeholder.svg?height=80&width=80",
      dateOfBirth: clinicPatient.dateOfBirth
        ? moment(clinicPatient.dateOfBirth).format("YYYY-MM-DD")
        : "",
      bloodType: clinicPatient.bloodType || "",
      medicalHistory: Array.isArray(clinicPatient.medicalHistory)
        ? clinicPatient.medicalHistory.join(", ")
        : clinicPatient.medicalHistory || "",
      allergies: clinicPatient.allergies || "",
      currentMedication: clinicPatient.currentMedication || "",
      insuranceProvider: clinicPatient.insuranceProvider || "",
      insuranceInfo: clinicPatient.insuranceInfo || "",
      doctorRef: (clinicPatient?.doctorRef as any)?._id || "",
      clinicRef: (clinicPatient?.clinicRef as any)?._id || "",
      address: {
        street: clinicPatient.address?.street || "",
        city: clinicPatient.address?.city || "",
        state: clinicPatient.address?.state || "",
        zipCode: clinicPatient.address?.zipCode || "",
        country: clinicPatient.address?.country || "USA",
      },
    });
    if (clinicPatient.profilePicture)
      setProfileImage(clinicPatient.profilePicture);
  }, [clinicPatient]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await dispatch(
        updateClinicPatient({
          id: patientId,
          patientData: payload as unknown as UpdateClinicPatientRequest,
        })
      ).unwrap();
      toast({ title: "Success", description: "Patient updated!", variant: "default" });
      router.push("/clinic/patients/view/" + patientId);
    } catch (err: any) {
      toast({
        title: "Error",
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-[hsl(var(--muted-foreground))]">
                  Update clinicPatient information and medical details
                </p>
              </div>
            </div>
          </div>
